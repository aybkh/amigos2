...
function SocialBtn({ href, label, children }) {
  if (!href) {
    return (
      <span className="footer-mobile-social-disabled" aria-label={label} title={label}>
        {children}
      </span>
    )
  }
...
export default function FooterMobile({ siteInfo }) {
  ...
  return (
    <footer className="footer-mobile">
      <div className="footer-mobile-container">
        ...
        {/* Fila 1: Logo + tagline en línea */}
        <div className="footer-mobile-row-1">
          <img src="/amigos2-logo-2-1.png" alt="Amigos2" className="footer-mobile-logo" />
          <span className="footer-mobile-tagline">
            {t(lang, 'ui.footer.tagline')} · Lloret de Mar
          </span>
        </div>
        ...
        <div className="footer-mobile-contact">
          ...
          {phone && email && <span className="footer-mobile-separator">·</span>}
        ...
        {/* Fila 4: Legales en línea con separadores */}
        <div className="footer-mobile-legal">
          {legalLinks.map(({ labelKey, modal }, i, arr) => (
            <span key={modal} className="footer-mobile-legal-item">
              <button type="button" onClick={() => openLegal(modal)}>
                {t(lang, labelKey)}
              </button>
              {i < arr.length - 1 && <span className="footer-mobile-separator">|</span>}
            </span>
          ))}
        </div>

        {/* Fila 5: Copyright en línea */}
        <div className="footer-mobile-bottom">
          <span>© 2026 Amigos2</span>
          <span className="footer-mobile-separator">·</span>
          <span>Lloret de Mar</span>
          <span className="footer-mobile-separator">·</span>
          <span className="footer-mobile-credits">
            {t(lang, 'ui.footer.designed_by')}{' '}
            <a href="https://ayoubjerari.com" target="_blank" rel="noopener noreferrer">AyoubDev</a>
          </span>
        </div>
